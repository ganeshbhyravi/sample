(function ($) {
	...
	$.num = function(v, defaultValue){
		v = Number($.isEmpty(v) || $.isArray(v) || typeof v == 'boolean' || (typeof v == 'string' && $.trim(v).length == 0) ? NaN : v);
		return isNaN(v) ? defaultValue : v;
	};
	$.isSafari = function() {
		return true == $.browser.safari;
	};
	$.isIE = function() {
	...
	$.strLeftPad = function (val, size, ch) {
	...
	(function() {
		...
		$.extend(Date, {
			...
			formatCodes : {
				...
				w: "this.getDay()",
				z: "this.getDayOfYear()",
				W: "$.strLeftPad(this.getWeekOfYear(), 2, '0')",
				F: "Date.monthNames[this.getMonth()]",
				m: "$.strLeftPad(this.getMonth() + 1, 2, '0')",
				...
				G: "this.getHours()",
				h: "$.strLeftPad((this.getHours() % 12) ? this.getHours() % 12 : 12, 2, '0')",
				H: "$.strLeftPad(this.getHours(), 2, '0')",
				i: "$.strLeftPad(this.getMinutes(), 2, '0')",
				s: "$.strLeftPad(this.getSeconds(), 2, '0')",
				u: "$.strLeftPad(this.getMilliseconds(), 3, '0')",
				O: "this.getGMTOffset()",
				...
				var code = [
					...
					"if(results){",
						...
						"}else{",
							...
							// date calculations (note: these calculations create a dependency on $.num())
							"y = y >= 0? y : $.num(def.y, dt.getFullYear());",
							"m = m >= 0? m : $.num(def.m - 1, dt.getMonth());",
							"d = d >= 0? d : $.num(def.d, dt.getDate());",

							// time calculations (note: these calculations create a dependency on $.num())
							"h  = h || $.num(def.h, dt.getHours());",
							"i  = i || $.num(def.i, dt.getMinutes());",
							"s  = s || $.num(def.s, dt.getSeconds());",
							"ms = ms || $.num(def.ms, dt.getMilliseconds());",
			...
			parseCodes : {
				...
				O: {
					...
					c:[
						...
						"o = ((-12 <= (hr*60 + mn)/60) && ((hr*60 + mn)/60 <= 14))? (sn + $.strLeftPad(hr, 2, '0') + $.strLeftPad(mn, 2, '0')) : null;\n" // -12hrs <= GMT offset <= 14hrs
					].join("\n"),
		...
		$.extend(Date.prototype, {
			...
			getGMTOffset : function(colon) {
				return (this.getTimezoneOffset() > 0 ? "-" : "+")
					+ $.strLeftPad(Math.floor(Math.abs(this.getTimezoneOffset()) / 60), 2, "0")
					+ (colon ? ":" : "")
					+ $.strLeftPad(Math.abs(this.getTimezoneOffset() % 60), 2, "0");
			},
		...
		// private
		if ($.isSafari() && (navigator.userAgent.match(/WebKit\/(\d+)/)[1] || NaN) < 420) {
			$.extend(Date.prototype, {
